...
# ---------- Minimal, deterministic runner per page ----------
def _run_module_or_files(preferred_module: str | None, *, func_candidates, file_candidates):
    """
    Try importing preferred_module and calling one of func_candidates.
    If that fails, execute the FIRST existing script in file_candidates.
    No cross-page fallbacks are allowed here.
    """
    if preferred_module:
        try:
            ...
            st.info(f"Module not found: {preferred_module}. Falling back to script.")
        except Exception as e:
            st.warning(f"Issue importing {preferred_module}: {e}. Falling back to script.")
    ...
    for p in file_candidates:
        ...
        if p.exists():
            try:
                ...
                st.error(f"Failed to execute {p.name}: {e}")
    st.error("Page script/module not found. Please check your repo layout.")

# ---------- Page renderers (isolated) ----------
def render_transaction_predictor():
    _run_module_or_files(
        "src.pages.transaction_predictor",
        func_candidates=("transaction_predictor", "main", "run", "app", "render", "show"),
        file_candidates=[
...
def render_fraud_detection_app2():
    _run_module_or_files(
        "src.pages.fraud_detection_app2",
